  ...
  <div>
    ...
    <div class="row no-gutters">
      <div class="col-6 col-sm-4 col-md-3 px-2 pb-4" v-for="(chatbot, c_idx) in chatbots" :key="c_idx">
        <Cartel :title="'Evalúa tu nivel en ' + chatbot.name" :image='chatbot.image' :callback="() => select(chatbot)" />
      </div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    this.loading_message = 'Cargando evaluaciones por unidad'
    this.chatbots = await getChatbotsBySession(session_id);
    this.loading = false;
  },
  methods: {
    select(chatbot) {
      redirect("pre-evaluation", { chatbot_id: chatbot._id.$oid });
    }
  },
  components: {
    Cartel,
